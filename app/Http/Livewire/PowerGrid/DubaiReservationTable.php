...
final class DubaiReservationTable extends PowerGridComponent
{
    use ActionButton;
    use WithExport;
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        ...
        return PowerGrid::eloquent()
            ...
            ->addColumn('client_phone', fn (Reservation $model) => strtolower($model->clientInfo->phone ?? 'N/A') )
...
//            Filter::datepicker('reservation_date'),
            ...
            Filter::select('place_name', 'place_id')
                ->dataSource(Place::where("id", 1)->get())
                ->optionValue('id')
                ->optionLabel('title'),

            Filter::select('room_name', 'room_id')
                ->dataSource(Room::where("place_id", 1)->get())
                ->optionValue('id')
                ->optionLabel('title'),
