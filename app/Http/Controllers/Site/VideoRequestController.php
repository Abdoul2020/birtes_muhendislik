...
class VideoRequestController extends Controller
{
    ...
    public function store(VideoRequestForm $request)
    {
        ...
        if ($request->get('payment_method') == 2) {
            try {
                ...
                if (isset($paymentResponse['reference'])) {
                    ...
                    // If required keys are missing in the response, throw an exception
                    throw new Exception('Payment response is missing required data.');
                }
            } catch (Exception $e) {
        ...
        return (isset($this->ORDER_PAGE_URL))
           ? redirect()->away($this->ORDER_PAGE_URL)
