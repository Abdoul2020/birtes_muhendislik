...
class ResimgalleriController extends Controller
{
    ...
    public function index()
    {
        ...
        $rooms = Resimgalleri::status()
            ->orderBy('order')
            ->get(['id', 'title', 'place_id', 'order', 'status']); 
            
            

            $mainProjects = Projects::status()->orderBy('order')->get(['id', 'title', 'order', 'status']);

        return view('pages.resimgalleri.index', compact('rooms', 'places', 'mainProjects'));

    }

    public function getRooms($place_id)
    {
        $rooms = Resimgalleri::where('place_id', $place_id)->status()->orderBy('order')->get(['id', 'title', 'place_id', 'order', 'status']);
    ...
    public function store(Request $request)
    {
        ...
        $params_poster = [
            "name" => "poster",
            "dir" => "uploads/resimgalleri/{$room->id}/",
        ...
        session()->flash('success', "Yeni Proje Resim Gallerisi başarıyla eklendi!");

        return redirect()->route('admin.ressimgalleri.index');
    ...
    {
        $room = Resimgalleri::findOrFail($id);

        // dd($room->price);

        $rooms = Resimgalleri::withoutTrashed()->get();
        $places = Place::withoutTrashed()->get();
        $hours = Hour::withoutTrashed()->get();

        $mainProjects = Projects::status()->orderBy('order')->get(['id', 'title', 'order', 'status']);

        return view('pages.resimgalleri._edit', [
            'room' => $room,
            // 'hour' => $room->hour,
            'rooms' => $rooms,
            'places' => $places,
    ...
    public function update(Request $request, $id)
    {
        ...
        session()->flash('message', "Proje Resim Gallerisi detayları güncellendi!");
    ...
    {
        $record = Resimgalleri::findOrFail($id);
