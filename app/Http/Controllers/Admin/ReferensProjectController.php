...
class ReferensProjectController extends Controller
{
    use UploadImage;
    ...
    public function index()
    {
        ...
        $rooms = ReferensProject::status()
            ->orderBy('order')
            ->get(['id', 'title', 'place_id', 'order', 'status']);

            $maincategories = MainCategory::withoutTrashed()->get(['id', 'title']);

        return view('pages.referensProject.index', compact('rooms', 'places', 'maincategories'));

    }

    public function getRooms($place_id)
    {
        $rooms = ReferensProject::where('place_id', $place_id)->status()->orderBy('order')->get(['id', 'title', 'place_id', 'order', 'status']);
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.referensproject.index');
    }
    ...
    {
        $room = ReferensProject::findOrFail($id);

        // dd($room->price);

        $maincategories = MainCategory::withoutTrashed()->get(['id', 'title']);

        // $rooms = ReferensProject::withoutTrashed()->get();
        // $places = Place::withoutTrashed()->get();
        // $hours = Hour::withoutTrashed()->get();

        return view('pages.referensProject._edit', [
            'room' => $room,
            // 'hour' => $room->hour,
            'rooms' => $maincategories,
            // 'places' => $places,
            // 'hours' => $hours
        ]);
    }
    ...
    public function destroy($id)
    {
        ...
        $record = ReferensProject::findOrFail($id);
        $record->delete();
        return redirect()->back();
    }
}
