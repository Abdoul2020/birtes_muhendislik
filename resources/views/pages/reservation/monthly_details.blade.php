<x-default-layout>
    ...
    @php
    // Example URL from request
    $path = request()->path(); // "admin/reservations/dubai/2024-03/list"

    // 1️⃣ Extract the YYYY-MM with regex
    if (preg_match('#/reservations/[^/]+/([0-9]{4}-[0-9]{2})/#', $path, $matches)) {
    $yearMonth = $matches[1]; // e.g. "2024-03"
    [$year, $month] = explode('-', $yearMonth);
    $dateObj = DateTime::createFromFormat('!m', $month);
    $monthName = strftime('%B', $dateObj->getTimestamp());
    setlocale(LC_TIME, 'tr_TR.UTF-8'); // ensure Turkish locale
    $monthName = ucfirst(strftime('%B', $dateObj->getTimestamp())); // e.g. "Mart"
    $label = "{$monthName} {$year} ait olan Kiralamalar"; // final label
    } else {
    $label = 'Kiralamalar';
    }
    @endphp
        ...
        <ul
            ...
            <li class="nav-item">
                <a class="nav-link text-active-primary pb-4 active" data-bs-toggle="tab"
                    href="#published_contents"> {{ $label }} </a>
            </li>
            <!--end:::Tab item-->
                    ...
                    <div class="card card-flush card-dashed">
                        ...
                        <div class="card-body pt-10">
                            ...
                            <table id="reservation_yearly" class="table table-striped border rounded gy-5 gs-7">
                                ...
                                $isDubai = Str::contains($currentPath, 'tum_urunler');
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @php
                                        $basePath = request()->is('*/tum_urunler/*') ? 'dubai' : 'abudhabi';
    ...
    <script>
        ...
        function redirectToDailyList(button) {
            ...
            var url = `/admin/kiralama/tum_urunler/details/${date}/list`;
