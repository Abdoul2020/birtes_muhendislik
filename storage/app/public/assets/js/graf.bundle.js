...
var KTCreateReservation = (function () {
    ...
    var initReservationDate = function () {
        var reservationDate = $(
            form.querySelector('[name="reservation_date"]')
        );
    ...
    var initReservationDateReturn = function () {
        var reservationDate = $(
            form.querySelector('[name="reservation_date_return"]')
        );
        reservationDate.flatpickr({
            enableTime: false,
            dateFormat: "Y-m-d",
    ...
    var initReservationHour = function () {
        var reservationHour = $(
            form.querySelector('[name="reservation_hour"]')
        );
    ...
    var initStepper = function () {
        ...
        stepperObj.on("kt.stepper.next", function (stepper) {
            ...
            var validator = validations[stepper.getCurrentStepIndex()]; // get validator for current step
        ...
        formSubmitButton.addEventListener("click", function (e) {
            console.log("errorDate", e);
            console.log("validError", validations[4]);
            console.log("whyyyNothis", e);

            // Validate form before change stepper step
            var validator = validations[4]; // get validator for last form

            validator.validate().then(function (status) {
                console.log("notvailderr!", status);
                ...
                if (status == "Valid") {
                    ...
                    console.log("formhre", form);
                    // Simulate ajax request
                    axios
                        .post(form.getAttribute("action"), new FormData(form))
                        .then(function (response) {
                            console.log("seeRESponses:", response);
    ...
    return {
        ...
        init: function () {
            ...
            stepper = document.querySelector(
                "#kt_modal_create_reservation_stepper"
            );
...
var KTReservationList = (function () {
    ...
    var initReservationTable = function () {
        ...
        $("input[type=checkbox][name=status]").change(function () {
            ...
            $(this).is(":checked")
                ? $("span.status").html("Active")
                : $("span.status").html("Passive");
        });
    ...
    var handleDeleteRows = () => {
        ...
        deleteButtons.forEach((d) => {
            ...
            d.addEventListener("click", function (e) {
                ...
                Swal.fire({
                    text:
                        "Are you sure do you wanna delete it " +
                        recordName +
                        " record?",
                    icon: "warning",
                ...
                if (result.value) {
                    ...
                    checkboxes.forEach((c) => {
                        if (c.checked) {
                            ...
                            axios
                                .delete("authors/" + item, { author: item })
                                .then(function (response) {
                                    $("div.card").removeClass(
                                        "overlay overlay-block"
                                    );
    ...
    if (document.getElementById("kt_table_reservation") != null) {
        ...
        $(document).on("click", "#updateButton", function () {
            ...
            if (dataID) {
                // Simulate ajax request
                axios
                    .get(baseURL + "admin/authors/" + dataID)
                    .then(function (response) {
                        var result = response.data.result[0];
                        // console.table(result);

                        const name = element.querySelector("input[name=name]");
                        const surname = element.querySelector(
                            "input[name=surname]"
                        );
                        const title =
                            element.querySelector("input[name=title]");
                        const position = element.querySelector(
                            "input[name=position]"
                        );
                        const label =
                            element.querySelector("input[name=label]");
                        const info = element.querySelector(
                            "textarea[name=info]"
                        );
                        const admin_id = element.querySelector(
                            "select[name=admin_id]"
                        );
                        const profile =
                            element.getElementsByClassName("profileImage");
                        const cover =
                            element.getElementsByClassName("coverImage");

                        $(profile).css({
                            "background-image":
                                "url(" + baseURL + result.profilePicUrl + ")",
                        });
                        $(cover).css({
                            "background-image":
                                "url(" + baseURL + result.headerImageUrl + ")",
                        });
                        ...
                        $(admin_id)
                            .val(result.admin_id)
                            .trigger("change")
                            .attr("data-placeholder", result.admin_id);

                        const isConsulting = element.querySelector(
                            "input[name=isConsulting]"
                        );
                        result.isConsulting === 1
                            ? $("span.isConsulting").html("Evet")
                            : $("span.isConsulting").html("Hayır");
                        result.isConsulting === 1
                            ? $(isConsulting).val(1).attr("checked", true)
                            : $(isConsulting).val(0).removeAttr("checked");

                        const isShowing =
                            element.querySelector("input[name=status]");
                        result.isShowing === 1
                            ? $("span.isShowing").html("Evet")
                            : $("span.isShowing").html("Hayır");
                        result.isShowing === 1
                            ? $(isShowing).val(1).attr("checked", true)
                            : $(isShowing).val(0).removeAttr("checked");
                    })
                    .catch(function (error) {
                        let dataMessage = error.message;
                        let dataErrors = error.errors;
...
var RoomList = (function () {
    if (document.getElementById("kt_table_room") != null) {
        ...
        var toolbarSelected;
        var selectedCount;
        var deleteAction = document.getElementsByTagName(
            "data-kt-table-filter"
        );
